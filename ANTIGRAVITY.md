# Project: 표준 ERP 포탈 (Antigravity 개발 지침)
...
- Database: 현재 브라우저 localStorage / 인메모리 기반 프로토타입, 향후 REST API + PostgreSQL 전환
...
## Coding Rules
- 기존 Claude/Codex 개발지침 및 코드 스타일과 네이밍을 승계한다.
...
- 모든 태스크는 '최소 클릭, 최대 효율' 원칙에 따라 수행하며, 한국어 결과 데이터 및 핵심 로직 위주로 보고한다.

## Verification Matrix
| 변경 유형 | 필수 검증 |
|---|---|
| 로직 | 관련 단위 테스트 / E2E 시나리오 |
...
## Memory
- 이 프로젝트 고유 사실은 `.agents\memory\MEMORY.md`에 있다.
